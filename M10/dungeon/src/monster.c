...
void moveTowards(const Game *game, Creature *monst) {
    ...
    if(game->position.x > monst->pos.x) {
        x++;
    } else if((game->position.x < monst->pos.x)) {
        x--;
    }    
    if((game->position.x == x && game->position.y == y) || !(isBlocked(game, x,y)) ){
        monst->pos.x = x;
        return;
    }
    x=monst->pos.x; // ??????
    if(game->position.y > monst->pos.y){
        y++;
    } else if((game->position.y < monst->pos.y)){
        y--;
    }
    if((x == game->position.x && y == game->position.y) ||!(isBlocked(game, x,y))){
        monst->pos.y = y;
        return;
    }*/
    int y = monst->pos.y;
    ...
    if(!(isBlocked(game, x, y)||(game->position.y == y && game->position.x == x))){
        monst->pos.x=x;
    return;
    }
    x=monst->pos.x;
...
void monsterAction(Game *game) {
    ...
    while (i<game->numMonsters){
        ...
        if (game->monsters[i].hp > 0){
            ...
            double erox = game->monsters[i].pos.x - game->position.x;
            double eroy = game->monsters[i].pos.y - game->position.y;

            if(erox * erox + eroy * eroy < 1.414){
...
void createMonsters(Game *game) {
    ...
    for (unsigned i = 0; i < game->opts.numMonsters;)
    {
