...
// Does not compare characters with NULL names
...
struct character *doSomething(char* inp, struct character *array) 
{
    ...
    else if(inp[0] == 'H') 
    {
        ...
        printf("%s attacked against %s with a %s\n", array[a].name, array[d].name, array[a].weapon);
        int damage;
        ...
        {
            printf("%s lost %d hitpoints and %s gained the same amount of xp\n%s has %d hitpoints left\n", array[d].name, damage, array[a].name, array[d].name, array[d].hp);
        }
        return array;
    }
    ...
    else if(inp[0] == 'L') 
    {
        ...
        qsort(a, i, sizeof(struct character), compareChar2);// Put dead characters to the end
